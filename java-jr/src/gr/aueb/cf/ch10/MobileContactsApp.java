...
public class MobileContactsApp {
    ...
    final static String[][] contacts = new String[500][3]; //Επώνυμο, Όνομα, Τηλέφωνο
    static int pivot = -1;  //Δείχνει στην τρέχουσα θέση και το που βρισκόμαστε στον πίνακα
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            if (s.matches("[qQ]")) quit = true;
                ...
                try {
                    choice = Integer.parseInt(s);  //δίνει NumberFormatException υποκλάση του Illegal
                    if (!isValid(choice)) {
                    ...
                    System.out.println(e.getMessage());  //Δίνει τα μηνύματα που έχουμε δώσει στα exceptions στις μεθόδους
                }
            }
        } while (!quit);
    }
    ...
    public static String[] getOneContactService(String phoneNumber) {
        try {
            ...
            if (contact.length == 0) {  //ελέγχω αν βρέθηκε το contact
    ...
    public static String[][] getAllContactsService() {
        ...
        try{
            ...
            if (contactsList.length == 0) { //Αν είναι κενός ο πίνακας επαφών throw new exception
    ...
    public static void insertContactService(String firstname, String lastname, String phoneNumber) {
        try {
            if (!(insert(firstname, lastname, phoneNumber))) {  //αν δεν είναι πετυχημένο το insert δηλαδή false δημιουργεί argument
    ...
    //με το που εμφανίζεται η update σε if ως boolean καλείται και εκτελείται αυτόματα παραπάνω και παρακάτω
    ...
    /**
     * Βοηθητική μέθοδος για τις υπόλοιπες CRUD
     * @param phoneNumber
     * @return
     */
    ...
    public static boolean insert(String firstname, String lastname, String phoneNumber) {
        if (isFull(contacts)) {   //ελέγχω αν είναι γεμάτος ο πίνακας
            return false;
        }

        if (getIndexByPhoneNumber(phoneNumber) != -1) {  //σημαίνει ότι βρέθηκε το τηλ
            return false;
        }
    ...
    public static boolean update(String oldPhoneNumber, String firstname, String lastname, String newPhoneNumber) {
        ...
        if (positionToUpdate == -1) {  //σημαίνει ότι δεν βρέθηκε
            return false;
            //return new String[] {};
        }
...
//        contact[2] = contacts[positionToUpdate][2];
    ...
    public static String[] delete(String phoneNumber) {
        ...
        //αντιγράφουμε τα στοιχεία που σβήνουμε στον contact
//        for (int i = 0; i < contact.length; i++) {
//            contact[i] = contacts[positionToDelete][i];
//        }
        System.arraycopy(contacts[positionToDelete], 0, contact, 0, contact.length);

        //μεταθέτουμε τα στοιχεία του πίνακα μια θέση πάνω, ελέγχοντας αν είναι γεμάτος ο πίνακας
        //μεταφέρει όλα τα στοιχεία από το positionToDelete + 1 πάλι στον contacts από τη θέση
        //positionToDelete γιά pivot - positionToDelete στοιχεία
    ...
    public static String[] getContactByPhoneNumber(String phoneNumber) {
        ...
        // αν δεν βρεθεί επιστρέφουμε κενό
        if (positionToReturn == -1) {
            return new String[] {};
        }

        //επιστρέφουμε μια γραμμή του δυσδιαστατου πίνακα σαν απλό πίνακα
    ...
    public static boolean isFull(String[][] arr) {
        return pivot == arr.length - 1; //Pivot δείχνει τελευταία θέση
    }
    ...
    public static void log(Exception e, String... message) {
        try (PrintStream ps = new PrintStream(new FileOutputStream(path.toFile(), true))) {
            ...
            ps.printf("%s", (message.length == 1) ? message[0] : ""); //Αν δωθεί 1 message (message.length == 1) εκτύπωσε το, αλλιώς τίποτα
