...
public class CryptoApp {
    ...
    /**
     * ArrayList είναι σαν μονοδιάστατο πίνακα χωρίς καθορισμένο μέγεθος
     */
    public static ArrayList<Integer> encrypt(String s, int key) {
        ArrayList<Integer> encrypted = new ArrayList<>();
        char ch; //Για να διαβάσουμε το string
        int i;
        ...
        i = 1;
        while ((ch = s.charAt(i)) != '#') {   //To string τελειώνει με #
            encrypted.add(cipher(ch, prev, key));
            prev = cipher(ch, prev, key);
            i++;
        }

        encrypted.add(-1);  //Το hash τέλους μετατρέπεται σε -1
        return encrypted;
    }
    ...
    public static int cipher(char ch, int prev, int key) {
        if (prev == -1) return ch;    //Υπονοείται ότι μετατρέπεται σε int
        return (ch + prev) % key;
    }
