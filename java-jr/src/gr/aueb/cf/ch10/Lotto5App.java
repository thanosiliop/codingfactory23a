...
public class Lotto5App {
    ...
    public static void main(String[] args) {
        try (Scanner in = new Scanner(new File("C:/tmp/lotto5in.txt"));
            ...
            int pivot = 0;   //Δείχνει μέχρι που ο πίνακας έχει στοιχεία (στην πρώτη ελεύθερη)
            int[] result = new int[5]; //Μπαίνει η κάθε πεντάδα
            int num; //Για να διαβάζω εναν εναν αριθμό
            int window; //Πόσες θέσεις μετακινούνται

            while((num = in.nextInt()) != -1 && pivot <= 48) {  //Γιατί μπορεί το αρχείο να έχει πάνω απο 49 ψηφία
                inputNumbers[pivot++] = num;
                //pivot++;
            }

            //pivot δίνει την ελεύθερη θέση γι αυτό δεν δίνουμε pivot + 1
            int[] numbers = Arrays.copyOfRange(inputNumbers, 0, pivot); //Αντιγράφουμε μόνο τον πίνακα που έχει νούμερα
            Arrays.sort(numbers); //Σορτάρουμε asc τον numbers

            //pivot είναι το πλήθος των στοιχείων επειδή ξεκινάνε απ'το 0
